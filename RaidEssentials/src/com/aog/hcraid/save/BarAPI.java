...
public class BarAPI {
        ...
        private class LazyPacket { //THANKS CAPTAINBERN :D
            ...
            public void send(Player player){
                try {
                	...
                	Raid.UTIL.getPlugin().getLogger().warning("Failed to send packet to player: " + player.getName() + "!");
                }
            }
        }
    ...
    public static void displayBar(final Player player, String msg, float percent, int seconds, boolean override){
        ...
        }.runTaskLater(Raid.UTIL.getPlugin(), (seconds * 20));
    }
        ...
        new BukkitRunnable(){
            ...
            public void run(){
                ...
                if((loadUp ? dragon.getHealth() < FakeDragon.MAX_HEALTH : dragon.getHealth() > 0)){
                    ...
                    }.runTaskLater(Raid.UTIL.getPlugin(), 40L);
                }
            }
        }.runTaskTimer(Raid.UTIL.getPlugin(), 0L, 1L);
    }
